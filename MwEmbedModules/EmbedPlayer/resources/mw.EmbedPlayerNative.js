...
mw.EmbedPlayerNative = {
	...
	addPlayScreenWithNativeOffScreen: function () {
		...
		this.$interface.find( '.play-btn-large' ).click( function () {
			localThis.$interface.find( '.play-btn-large' ).hide();
			localThis.addPlayerSpinner();
			localThis.hideSpinnerOncePlaying();
		});
	...
	*/
	embedPlayerHTML : function () {
		var localThis = this;
		var vid = localThis.getPlayerElement();
		...
		localThis.bufferStartFlag = false;
		localThis.bufferEndFlag = false;

		$( this ).html(
			localThis.getNativePlayerHtml()
		);
	...
	*/
	postEmbedActions: function () {
		...
		localThis.applyMediaElementBindings();
	...
	*/
	doNativeSeek: function ( percent, callback ) {
		...
		if ( (navigator.userAgent.indexOf('Chrome') === -1) && localThis.playerElement.seeking ) {
			return;
		}
		...
		this.setCurrentTime( ( percent * this.duration ) , function () {
			...
			localThis.currentTime = localThis.getPlayerElement().currentTime;
			// Done seeking ( should be a fallback trigger event ) :
			if ( localThis.seeking ) {
				localThis.seeking = false;
				$( localThis ).trigger( 'seeked' );
			}
			// restore iPad video position:
			localThis.restorePlayerOnScreen();
	...
	*/
	doPlayThenSeek: function ( percent ) {
		...
		var readyForSeek = function () {
			...
			if ( localThis.playerElement && localThis.playerElement.duration ) {
				localThis.doNativeSeek( percent, function () {
				...
				if ( retryCount < 800 ) {
					...
					mw.log( 'EmbedPlayerNative:: Error: doPlayThenSeek failed :' + localThis.playerElement.duration);
				}
			}
		};
		readyForSeek();
	...
	*/
	setCurrentTime: function ( seekTime , callback, callbackCount ) {
		...
		if ( vid.readyState < 1 ) {
			...
			setTimeout( function () {
				...
				if ( $( localThis ).data('currentSeekTarget') !== seekTime ) {
					mw.log("EmbedPlayerNative:: expired seek target");
					return;
				}
				localThis.setCurrentTime( seekTime, callback , callbackCount+1);
			}, 100 );
			return;
		}
		...
		setTimeout(function () {
			...
			if ( $.isfunction ( callback ) ) {
				...
				if ( Math.abs( vid.currentTime - seekTime ) < 5 ) {
					...
					localThis.setCurrentTime( seekTime, callback , callbackCount++ );
				}
			}
		}, 5000);
		...
		try {
			localThis.currentSeekTargetTime = seekTime.toFixed( 2 );
			// use toFixed ( iOS issue with float seek times )
			vid.currentTime = localThis.currentSeekTargetTime;
		} catch ( e ) {
		...
		if (! vid.seeking ) {
			...
			vid.play();
			setTimeout(function () {
				localThis.waitForPositiveCurrentTime( function () {
	...
	waitForPositiveCurrentTime: function ( callback ) {
		...
		if ( vid.currentTime > 0 ) {
			...
			setTimeout(function () { localThis.waitForPositiveCurrentTime( callback ) }, 50 )
		}
	},
	/**
		...
		if ( vid ) {
			try {
				...
				$( vid ).bind( 'loadedmetadata' + switchBindPostfix, function () {
					...
					if ( !mw.isAndroid40() ) {
						localThis.duration = vid.duration;
					}
	...
	*/
	...
	onClipDone: function () {
		...
		$(this).unbind('onEndedDone.onClipDone').bind( 'onEndedDone.onClipDone', function () {
			localThis.addPlayScreenWithNativeOffScreen();
			// if not a legitmate play screen don't keep the player offscreen when playback starts:
			if ( !localThis.isImagePlayScreen() ) {
				localThis.keepPlayerOffScreenFlag =false;
			}
		});
