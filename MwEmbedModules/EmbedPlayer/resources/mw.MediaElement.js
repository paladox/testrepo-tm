...
mw.MediaElement.prototype = {
	...
	autoSelectSource: function () {
		...
		var setSelectedSource = function ( source ) {
			localThis.selectedSource = source;
			return localThis.selectedSource;
		};

		// Set via module driven preference:
		$( this ).trigger( 'onSelectSource', playableSources );

		if ( localThis.selectedSource ) {
			mw.log('MediaElement::autoSelectSource: Set via trigger::' + localThis.selectedSource.getTitle() );
			return localThis.selectedSource;
		}
		...
		if ( this.selectedSource ) {
			mw.log('MediaElement::autoSelectSource: Set via Adaptive HLS: source flavor id:' + localThis.selectedSource.getFlavorId() + ' src: ' + localThis.selectedSource.getSrc() );
