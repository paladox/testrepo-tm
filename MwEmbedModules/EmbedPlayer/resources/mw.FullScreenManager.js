...
mw.FullScreenManager.prototype = {
	...
	*/
	doFullScreenPlayer: function( callback ) {
        ...
        if( mw.config.get('EmbedPlayer.NewWindowFullscreen') && !screenfull &&
            !(mw.config.get('EmbedPlayer.EnableIpadNativeFullscreen') && mw.isIpad())){
            this.openNewWindow();
            return;
        }
		...
		var isIframe = (mw.config.get('EmbedPlayer.IsIframeServer' ) && mw.config.get('EmbedPlayer.IsFriendlyIframe')),
		...
		if( screenfull && screenfull.enabled(doc) && !mw.isMobileChrome() ) {
			...
			var escapeFullscreen = function( event ) {
				...
				var doc = ( mw.config.get('EmbedPlayer.IsIframeServer' ) && mw.config.get('EmbedPlayer.IsFriendlyIframe'))?
	...
	getFsTarget: function(){
		if( mw.config.get('EmbedPlayer.IsIframeServer' ) && mw.config.get('EmbedPlayer.IsFriendlyIframe')){
