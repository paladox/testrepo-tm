...
mw.PlayerControlBuilder.prototype = {
	...
	*/
	init: function ( embedPlayer ) {
		...
		if ( mw['PlayerSkin' + skinClass ] ) {
			...
			localThis = $.extend( true, { }, this, mw['PlayerSkin' + skinClass ] );
		}
		if (
				localThis.embedPlayer.mediaElement.getPlayableSources().length <= 1 &&
				localThis.supportedMenuItems.playerSelect
			) {
			delete localThis.supportedMenuItems.playerSelect;
		}
	...
	*/
	addControls: function () {
		...
		// Reset flags:
		localThis.displayOptionsMenuFlag = false;
		...
		if ( localThis.isOverlayControls() ) {
			$controlBar.hide();
		} else {
	...
	addControlComponents: function ( ) {
		...
		if ( embedPlayer.disablecontrols ) {
			embedPlayer.disablecontrols.split(',').forEach(function ( key ) {
				...
				localThis.supportedComponents[ key ] = false;
			});
		}
		...
		var addComponent = function ( componentId ) {
			if ( localThis.supportedComponents[ componentId ] ) {
				if ( localThis.availableWidth >= localThis.components[ componentId ].w ) {
					localThis.availableWidth -= localThis.components[ componentId ].w;
					// Check if position is defined, if not, place at end of known positions
					var position =  localThis.components[ componentId ].position ?
							localThis.components[ componentId ].position :
							largestPos + 1;
					...
					//mw.log(" availableWidth:" + localThis.availableWidth + ' ' + componentId + ' took: ' +  localThis.components[ componentId ].w )
				} else {
		...
		var addComponents = function () {
			...
			for(var i=0;i<components.length;i++) {
				$controlBar.append(
					localThis.getComponent( components[ i ].id )
				);
			}
		};
	...
	*/
	getAspectPlayerWindowCss: function ( windowSize ) {
		...
		var targetHeight = targetWidth * ( 1 / localThis.getIntrinsicAspect() );
		// Check if it exceeds the height constraint:
		if ( targetHeight > windowSize.height ) {
			targetHeight = windowSize.height;
			targetWidth = parseInt( targetHeight * localThis.getIntrinsicAspect() );
		}
		var offsetTop = 0;
	...
	*/
	doFullScreenPlayer: function ( /* callback */ ) {
		...
		if ( localThis.isOverlayControls() && !embedPlayer.isTouchDevice() ) {
			localThis.addFullscreenMouseMoveHideShowControls();
		}

		// Store the current scroll location on the iframe:
		$( embedPlayer ).trigger( 'fullScreenStoreVerticalScroll' );

		var fullScreenApi = window.fullScreenApi;

		if ( fullScreenApi.supportsFullScreen ) {
			localThis.preFullscreenPlayerSize = this.getPlayerSize();
			...
			var escapeFullscreen = function () {
				...
				if ( ! fullScreenApi.isFullScreen( window.document ) ) {
					localThis.restoreWindowPlayer();
				}
			};
			...
			fullScreenApi.requestFullScreen( fsTarget );
			...
			if ( $.browser.mozilla ) {
				localThis.fullscreenRestoreCheck = setInterval( function () {
					if ( fullscreenHeight && $(window).height() < fullscreenHeight ) {
						// Mozilla triggered size change:
						clearInterval ( localThis.fullscreenRestoreCheck );
						localThis.restoreWindowPlayer();
					}
					// set fullscreen height:
					if ( ! fullscreenHeight && localThis.preFullscreenPlayerSize.height !== $(window).height() ) {
	...
	doContextTargetFullscreen: function () {
		...
		$target.parents().each( function () {
			...
			if ( $parent.css( 'position' ) === 'absolute' ) {
				localThis.parentsAbsoluteList.push( $parent );
				$parent.css( 'position', 'static' );
			}
			if ( $parent.css( 'position' ) === 'relative' ) {
				localThis.parentsRelativeList.push( $parent );
				$parent.css( 'position', 'static' );
			}
		});
		...
		var updateTargetSize = function () {
			...
			localThis.embedPlayer.applyIntrinsicAspect();
		};
	...
	restoreContextPlayer: function () {
		...
		$doc.find( localThis.parentsAbsoluteList ).each( function () {
			$( this ).css( 'position', 'absolute' );
		} );
		$doc.find( localThis.parentsRelativeList ).each( function () {
	...
	doHybridNativeFullscreen: function () {
		...
		this.fsIntervalID = setInterval( function () {
			...
			if ( localThis.isInFullScreen() && !currentFS ) {
				// restore non-fullscreen player state
				localThis.inFullScreen = false;
				// Trigger the onCloseFullscreen event:
				$( localThis.embedPlayer ).trigger( 'onCloseFullScreen' );
				// stop polling for state change.
				clearInterval( localThis.fsIntervalID );
			}
		}, 250 );
	},
	...
	doDomFullscreen: function () {
		...
		if (
				$interface.offsetParent()[0].tagName &&
		...
		$interface.parents().each( function () {
			...
			if ( $( this ).css( 'position' ) === 'absolute' ) {
				localThis.parentsAbsolute.push( $( this ) );
	...
	*/
	restoreWindowPlayer: function () {
		...
		setTimeout( function () {
			window.scroll( 0, localThis.verticalScrollPosition );
		}, 100 );
	...
	restoreDomPlayer: function () {
		...
		var interfaceHeight = ( localThis.isOverlayControls() ) ?
			embedPlayer.getHeight() : embedPlayer.getHeight() + localThis.getHeight();
		...
		// Restore the player:
		embedPlayer.getInterface().css( {
			'width': localThis.preFullscreenPlayerSize.width,
			'height': localThis.preFullscreenPlayerSize.height
		});
		var topPos = {
			'position': localThis.windowPositionStyle,
			'z-index': localThis.windowZindex,
			'overlow': 'visible',
			'top': '0px',
			'left': '0px'
		};
	...
	*/
	addControlBindings: function ( ) {
		...
		var bindSpaceUp = function () {
			$(window).bind('keyup' + localThis.bindPostfix, function (e) {
				if ( e.keyCode === 32 ) {
		...
		var bindSpaceDown = function () {
			$(window).unbind( 'keyup' + localThis.bindPostfix );
		};
		...
		*/
		...
		if ( ! localThis.isOverlayControls() ) {
			...
			$( embedPlayer).bind( 'touchstart' + this.bindPostfix, function () {
				...
				clearTimeout( localThis.hideControlBarCallback );
				localThis.hideControlBarCallback = setTimeout( function () {
					localThis.hideControlBar();
				}, 60000 );
			...
			var hoverIntentConfig = {
					...
					'over': function () {
						...
						if ( mw.isIE9() ) {
							clearTimeout(localThis.hideControlBarCallback);
							localThis.hideControlBarCallback = false;
						}
			...
			if ( mw.isIE9() ) {
				...
				embedPlayer.getInterface().find( '.control-bar' ).hover(
					function () {
						localThis.onControlBar = true;
						embedPlayer.getInterface().find( '.control-bar' ).show();
					}, function () {
						if (!localThis.hideControlBarCallback) {
							localThis.hideControlBarCallback = setTimeout( function () {
									localThis.hideControlBar();
								}, 1000);
						}
		...
		if ( localThis.checkNativeWarning( ) ) {
			localThis.addWarningBinding(
	...
	addPlayerClickBindings: function () {
		...
		if ( this.supportedComponents.fullscreen ) {
			$( embedPlayer ).bind( "dblclick" + localThis.bindPostfix, function () {
	...
	*/
	hideControlBar : function () {
		...
		if ( localThis.displayOptionsMenuFlag || localThis.keepControlBarOnScreen ) {
			setTimeout( function () {
				localThis.hideControlBar();
			}, 200 );
			return;
		...
		if ( !hideDisableUi ) {
			...
			$targetWarning.append(
				...
				.click( function () {
					...
					localThis.addWarningFlag = false;
				} )
			);
			$targetWarning.append(
				$('<label />')
	...
	*/
	...
	aboutPlayerLibrary: function () {
		return $( '<div />' )
			.append(
				$( '<h2 />' )
					.text( mw.msg('mwe-embedplayer-about-library') ),
				$( '<span />')
					.append(
	...
	*/
	getShare: function ( ) {
		...
		$shareInterface.append(
			$( '<h2 />' )
			.text( mw.msg( 'mwe-embedplayer-sharelocalThis_video' ) )
		);
	...
	*/
	showDownload: function ( $target ) {
		this.showDownloadWithSources( $target );
	},
		...
		function addToSourceMenu( source ) {
			...
			$sourceMenu.append(
				$.getLineItem( source.getShortTitle() , icon, function () {
					...
					if ( ! localThis.embedPlayer.isStopped() ) {
						// Get the exact play time from the video element ( instead of parent embed Player )
						var oldMediaTime = localThis.embedPlayer.getPlayerElement().currentTime;
						var oldPaused =  localThis.embedPlayer.paused;
						// Do a live switch
	...
	components: {
		...
		*/
		'attributionButton': {
			...
			'position': 3,
			'o': function ( /* ctrlObj */ ) {
				var $icon, buttonConfig = mw.config.get( 'EmbedPlayer.AttributionButton');
		...
		*/
		'fullscreen': {
			...
			'o': function ( ctrlObj ) {
				...
				if (
						( mw.config.get('EmbedPlayer.IsIframeServer') && mw.isIpad3() ) ||
						mw.config.get( "EmbedPlayer.NewWindowFullscreen" ) ||
						(
							mw.config.get('EmbedPlayer.IsIframeServer') &&
							mw.config.get('EmbedPlayer.EnableIframeApi') === false
					) ) {
					// Get the iframe url:
