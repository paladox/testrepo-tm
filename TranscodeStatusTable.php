...
class TranscodeStatusTable {
	...
	public static function formatFromTranscodeKey( $key ) {
		$bits = explode( '.', $key, 2 );
		if ( count( $bits ) == 2 ) {
			return $bits[1];
		} else {
	...
	public static function getTranscodeRows( $file ){
		...
		uksort( $transcodeRows, function( $a, $b ) {
			$formatOrder = array( 'vp9.webm', 'webm', 'h264', 'ogv' );

			$aFormat = self::formatFromTranscodeKey( $a );
			$bFormat = self::formatFromTranscodeKey( $b );
