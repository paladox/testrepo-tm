...
class ApiTimedText extends ApiBase {
	...
	public function execute() {
		...
		if ( !$this->file ) {
			$this->dieUsage( 'File not found', 'timedtext-notfound' );
		}
		if ( !$this->file->isLocal() ) {
			$this->dieUsage( 'File not local', 'timedtext-notlocal' );
		}

		// Find subtitle [content] that goes with file
		$page = $this->findTimedText( $langCode, $params['trackformat'] );
		if ( !$page ) {
			$this->dieUsage( 'Requested language not found', 'timedtext-notfound' );
		}
	...
	function findTimedText( $langCode, $preferredFormat ) {
		$this->getTextHandler();
		$ns = $this->textHandler->getTimedTextNamespace();
		if ( !$ns ) {
			$this->dieUsage( 'No TimedText support', 'invalidconfig' );
		}

		$sourceFormats = [ 'vtt', 'srt' ];
