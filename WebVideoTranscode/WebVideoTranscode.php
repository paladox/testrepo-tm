...
class WebVideoTranscode {
	...
	*/
	...
	static public function getDerivativeFilePath( &$file, $transcodeKey){
		if ( MWInit::methodExists( $file, 'getTranscodesPath' ) ) {
			$path = $file->getTranscodesPath( $file->thumbName( array() ) );
		} else {
	...
	static public function getDerivativeSourceAttributes($file, $transcodeKey, $options = array() ){
		...
		if ( MWInit::methodExists( $file, 'getTranscodesUrl' ) ) {
			$transcodeUrl = $file->getTranscodesUrl( $thumbName );
		} else {
			$transcodeUrl = $file->getThumbUrl( $thumbName );
		}
		$transcodeUrlDir = dirname( $transcodeUrl );
		...
		// Setup the url src:
		$src = $transcodeUrlDir . '/' . rawurlencode( $file->getName() ) . '.' . $transcodeKey;
