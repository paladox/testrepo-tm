...
class WebVideoTranscode {
	...
	*/
	...
	static public function getDerivativeFilePath( &$file, $transcodeKey){
		if ( MWInit::methodExists( $file, 'getTranscodePath' ) ) {
			$path = $file->getTranscodePath( $file->thumbName( array() ) );
		} else {
	...
	static public function getDerivativeSourceAttributes($file, $transcodeKey, $options = array() ){
		...
		if ( MWInit::methodExists( $file, 'getTranscodeUrl' ) ) {
			$transcodeUrl = $file->getTranscodeUrl( $thumbName );
		} else {
