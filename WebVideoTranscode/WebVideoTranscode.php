	...
	static public function getMaxSizeWebStream(){
		...
		foreach( $wgEnabledTranscodeSet as $transcodeKey ){
			...
			if ( $wgEnabledTranscodeSet = "OGV_240P" || $wgEnabledTranscodeSet = "WebVideoTranscode::ENC_OGV_240P" ) {
					if( isset( self::$derivativeSettingsOGV_240P[$transcodeKey]['videoBitrate'] ) ){
						$currentSize = self::$derivativeSettingsOGV_240P[$transcodeKey]['maxSize'];
						if( $currentSize > $maxSize ){
							$maxSize = $currentSize;
						}
					}
			...
			if ( $wgEnabledTranscodeSet = "OGV_360P" || $wgEnabledTranscodeSet = "WebVideoTranscode::ENC_OGV_360P" ) {
					if( isset( self::$derivativeSettingsOGV_360P[$transcodeKey]['videoBitrate'] ) ){
						$currentSize = self::$derivativeSettingsOGV_360P[$transcodeKey]['maxSize'];
						if( $currentSize > $maxSize ){
							$maxSize = $currentSize;
						}
					}
	...
	static public function getDerivativeSourceAttributes($file, $transcodeKey, $options = array() ){
		...
		if ( $wgEnabledTranscodeSet = "OGV_160P" || $wgEnabledTranscodeSet = "WebVideoTranscode::ENC_OGV_160P" ) {
			...
			$fields = array(
					...
					'type' => self::$derivativeSettingsOGV_160P[ $transcodeKey ][ 'type' ],
