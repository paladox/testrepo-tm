...
class TimedMediaHandlerHooks {
	...
	public static function TimedMediaHandlerHash( &$hash, User $user, &$forOptions ) {
		if ( self::activeMode() === 'videojs' ) {
			if ( $user->getOption() === 'tmh-videojs' ) {
				$hash .= '!tmh-videojs';
				return true;
			}
		}
	...
	public static function onGetBetaFeaturePreferences( $user, &$prefs ) {
		global $wgTMHVideoJSBetaFeature, $wgExtensionAssetsPath;

		if ( $wgTMHVideoJSBetaFeature ) {
			$prefs['tmh-videojs'] = [
	...
	public static function activeMode() {
		global $wgTmhWebPlayer, $wgTMHVideoJSBetaFeature;
		$context = new RequestContext();
		if ( $wgTMHVideoJSBetaFeature && class_exists( 'BetaFeatures' ) &&
