...
// @codingStandardsIgnoreEnd
	 ...
	 */
	// @codingStandardsIgnoreStart
	public $_streamList = array();
	public $_streams = array();
	// @codingStandardsIgnoreEnd
	...
	function __construct( $fileLocation ) {
		...
		if ( !is_resource( $this->_filePointer ) ) {
			throw new OggException(
				"Couldn't Open File.  Check File Permissions.", OGG_ERROR_INVALID_FILE
			);
		}
	...
	// @codingStandardsIgnoreEnd
		...
		// Swap nibbles back
		$endianBin = strlen( $bigEndian );
		for ( $i = 0; $i < $endianBin; $i += 2 ) {
			$temp = $reversed[$i];
	...
	// @codingStandardsIgnoreEnd
		...
		while ( !feof( $this->_filePointer ) ) {
			...
			if ( $page === false ) {
				throw new OggException(
					"Cannot decode Ogg file: Invalid page at offset $this_page_offset", OGG_ERROR_UNDECODABLE
				);
			}
