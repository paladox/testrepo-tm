...
// @codingStandardsIgnoreEnd
	...
	function __construct( $streamSerial, $streamData, $filePointer ) {
		...
		$this->_avgBitrate =
			$this->_streamLength ? ( $this->_streamSize * 8 ) / $this->_streamLength : 0;
	}
	...
	// @codingStandardsIgnoreEnd
		$this->_decodeCommonHeader(
			OGG_VORBIS_IDENTIFICATION_HEADER, OGG_VORBIS_IDENTIFICATION_PAGE_OFFSET
		);
		...
		if ( $h['vorbis_version'] == 0 ) {
			...
			throw new OggException(
				"Stream is undecodable due to an invalid vorbis stream version.", OGG_VORBIS_ERROR_UNDECODABLE
			);
		}
		...
		if ( $h['audio_channels'] == 0 ) {
			throw new OggException(
				"Stream is undecodable due to zero channels.", OGG_VORBIS_ERROR_UNDECODABLE
			);
		} else {
		...
		if ( $h['audio_sample_rate'] == 0 ) {
			throw new OggException(
				"Stream is undecodable due to a zero sample rate.", OGG_VORBIS_ERROR_UNDECODABLE
			);
		} else {
		...
		if ( false == in_array( $blocksize_0, $valid_block_sizes ) ) {
			throw new OggException(
				"Stream is undecodable because blocksize_0 is $blocksize_0, which is not a valid size.",
		...
		if ( false == in_array( $blocksize_1, $valid_block_sizes ) ) {
			throw new OggException(
				"Stream is undecodable because blocksize_1 is not a valid size.", OGG_VORBIS_ERROR_UNDECODABLE
			);
		}
		...
		if ( $blocksize_0 > $blocksize_1 ) {
			throw new OggException(
				"Stream is undecodable because blocksize_0 is not less than or equal to blocksize_1.",
				OGG_VORBIS_ERROR_UNDECODABLE
			);
		}
		...
		if ( $h['framing_flag'] == 0 ) {
			throw new OggException(
				"Stream in undecodable because the framing bit is not non-zero.", OGG_VORBIS_ERROR_UNDECODABLE
			);
		}
		*/
	...
	function getBitrates() {
		return (
			array(
				"nom" => $this->_nomBitrate,
				"max" => $this->_maxBitrate,
				"min" => $this->_minBitrate,
				"avg" => $this->_avgBitrate
			)
		);
	}
