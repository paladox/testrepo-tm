...
// @codingStandardsIgnoreEnd
	...
	// @codingStandardsIgnoreEnd
		$this->_decodeCommonHeader(
			OGG_THEORA_IDENTIFICATION_HEADER, OGG_THEORA_IDENTIFICATION_PAGE_OFFSET
		);
		...
		if ( !$h ) {
			throw new OggException(
				"Stream is undecodable due to a truncated header.", OGG_ERROR_UNDECODABLE
			);
		}
		...
		if ( $h['VMAJ'] != 3 || $h['VMIN'] != 2 ) {
			throw new OggException(
				"Stream is undecodable due to an invalid theora version.", OGG_ERROR_UNDECODABLE
			);
		}
		...
		if ( $h['PICW'] > $this->_frameWidth || $h['PICH'] > $this->_frameHeight ) {
			throw new OggException(
				"Stream is undecodable because the picture width is greater than the frame width.",
				OGG_ERROR_UNDECODABLE
			);
		}
