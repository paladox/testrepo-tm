( function( $, mw, M, support ) {
	...
	var MediaOverlay = Overlay.extend( {
		...
		template: M.template.compile( '<div class="main">\n' +
			...
			'	{{>cancelButton}}' +
			'	<div class="details">\n' +
			'		<a class="mw-ui-button mw-ui-progressive button" href="#">{{detailsMsg}}</a>\n' +
			'	</div>\n' +
			'</div>', 'hogan' ),
		...
		postRender: function( options ) {
			...
			var $target = $( document.getElementById( this.options.title ) );
			var $content;
			...
			transformMediaElementsForOgvJs( $content[0] ).done( function( element ) {
				self.$el.find( '.media-wrapper' ).removeClass( 'loading' );
		...
		/*
		// For some reason enabling this *does* stop the playback, but breaks
		// subsequent invocations of the overlay. No idea why.
		hide: function( force ) {
			if ( this._player ) {
				// OgvJsPlayer doesn't yet know to stop when it's removed from the DOM,
		...
		*/
		...
		_positionImage: function() {
			...
			this.$el.find( '.media-wrapper' ).css( 'bottom', detailsHeight );
		}
	} );
