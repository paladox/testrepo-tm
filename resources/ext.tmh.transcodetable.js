...
$(document).ready(function () {
	...
	$('.mw-filepage-transcodereset a').click( function () {
		...
		buttons[ mw.msg('mwe-ok') ] = function () {
			...
			$( localThisDialog ).dialog( "option", "buttons", cancelBtn );
			...
			$.post( apiUrl, {
				...
				if ( data && data['success'] ) {
					...
					if ( data.error && data.error.info ) {
						$( localThisDialog ).text( data.error.info );
					} else {
						$( localThisDialog ).text( mw.msg( 'timedmedia-reset-error' ) );
					}
					var okBtn = {};
					okBtn[ mw.msg('mwe-ok') ] = function () { $(this).dialog("close"); }
					$( localThisDialog ).dialog( "option", "buttons", okBtn );
				}
			})
		};
