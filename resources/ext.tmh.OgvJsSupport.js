( function( $, mw ) {
	...
	var support = mw.OgvJsSupport = {
		 ...
		 */
		version: OgvJsVersion,

		/**
		 * Ensure that the OgvJsPlayer class is loaded before continuing.
		 *
		 * @return jQuery.Deferred
		 */
		loadOgvJs: function() {
			return support.loadClass( typeof OgvJsPlayer, 'ogvjs.js' );
		},

		/**
		 * Ensure that the OgvSwfPlayer class is loaded before continuing.
		 *
		 * @return jQuery.Deferred
		 */
		loadOgvSwf: function() {
			return support.loadClass( typeof OgvSwfPlayer, 'ogvswf.js' );
		},

		loadClass: function( classType, script ) {
			return $.Deferred( function( deferred ) {
				if ( classType === 'undefined' ) {
					$.ajax({
						dataType: 'script',
						cache: true,
						url: support.findScript( script )
					}).done(function() {
						deferred.resolve();
					});
				} else {
