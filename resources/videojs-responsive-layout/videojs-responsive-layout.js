...
var updateVideoJsLayout = function updateVideoJsLayout(layouter, playerWidth, controlBarWidth, controlWidth) {
  var progressWidth = _getElementOuterWidth(layouter.el.querySelectorAll('.vjs-progress-control')[0]);

  if (controlBarWidth > playerWidth) {
    makeSmaller(layouter.player);
  } else if ((_videoJs2['default'].hasClass(layouter.el, 'vjs-layout-x-small') || _videoJs2['default'].hasClass(layouter.el, 'vjs-layout-small')) &&
  // progressbar more that twice it's default size
  progressWidth > controlWidth * 2) {
    makeBigger(layouter.player);
  } else if (playerWidth > controlBarWidth + controlWidth && !_videoJs2['default'].hasClass(layouter.el, 'vjs-layout-x-small') && !_videoJs2['default'].hasClass(layouter.el, 'vjs-layout-small')) {
    makeBigger(layouter.player);
  }
};
